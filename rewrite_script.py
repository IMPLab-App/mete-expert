...
new_code = """from semilearn.core.utils.misc import count_parameters
...
def evaluate_and_plot(net, eval_loader, save_root, args, class_names, device, use_cuda, suffix_title):
    ...
    # rename for suffix
    mean_path_new = mean_path.replace("avg_prob_mean_matrix.csv", f"avg_prob_mean_matrix_{suffix_title}.csv")
    std_path_new = std_path.replace("avg_prob_std_matrix.csv", f"avg_prob_std_matrix_{suffix_title}.csv")
    mean_std_path_new = mean_std_path.replace("avg_prob_mean_std_matrix.csv", f"avg_prob_mean_std_matrix_{suffix_title}.csv")
    os.rename(mean_path, mean_path_new)
    os.rename(std_path, std_path_new)
    os.rename(mean_std_path, mean_std_path_new)
...
def main():
    ...
    balanced_dset = BasicDataset(balanced_data, balanced_targets, None, args.num_classes, False, weak_transform=eval_dset.weak_transform, strong_transform=None, onehot=False)
    ...
    lt_data = data[lt_indices]
    lt_targets = targets[lt_indices]
    
    lt_dset = BasicDataset(lt_data, lt_targets, None, args.num_classes, False, weak_transform=eval_dset.weak_transform, strong_transform=None, onehot=False)
    ...
    evaluate_and_plot(net, lt_loader, save_root, args, class_names, device, use_cuda, "balanced") # wait, lt_loader is wrong here. Let's fix this script.
